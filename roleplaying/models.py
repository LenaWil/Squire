...
class RoleplayingSystem(models.Model):
    ...
    rate_lore = models.IntegerField(validators=[MinValueValidator(1), MaxValueValidator(5)], blank=True, null=True)
...
class RoleplayingItem(Item):
    system = models.ForeignKey(
        RoleplayingSystem, related_name="items", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    local_file_name = models.CharField(
        max_length=32, blank=True, null=True, verbose_name="Filename (without extension)"
    )
    ...
    def clean(self):
        if self.local_file and self.external_file_url:
            raise ValidationError(
                "You can not set both a digital file and an external file location", code="duplicate_location"
            )
