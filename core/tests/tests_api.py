...
class MartorImageUploadTest(TestCase):
    ...
    def _test_image_upload(
        ...
        expected_json_error_message=None,
        ...
        with open(filename, "rb") as fp:
            ...
            # Set up POST data
            data = {"martor_image_upload_content_type_id": content_type_id, "markdown-image-upload": fp}
    ...
    def test_upload_invalid_file_key(self):
        ...
        with open(self.valid_filename, "rb") as fp:
            response = self.client.post(self.martor_image_upload_url, {"foo": fp})  # Should be markdown-image-upload
    ...
    def test_upload_invalid_contenttype_object_id_combination(self):
        ...
        self._test_image_upload(
            self.valid_filename,
            content_type_id="I am a string",
            expected_status_code=400,
            expected_json_error_message="Invalid content_type/object_id combination",
        )
    ...
    def test_upload_invalid_content_type(self):
        ...
        self._test_image_upload(
            ...
            expected_status_code=400,
            expected_json_error_message="Cannot upload MarkdownImages for this model",
        )
    ...
    def test_upload_max_filesize(self):
        ...
        self._test_image_upload(
            self.valid_filename, expected_status_code=400, expected_json_error_message="Maximum image file size is"
        )
    ...
    def test_upload_invalid_image(self):
        ...
        self._test_image_upload(
            self.invalid_filename, expected_status_code=400, expected_json_error_message="Bad image format"
        )
    ...
    def test_upload_anonymous_user(self):
        ...
        self.client.logout()
        self._test_image_upload(self.valid_filename, expected_status_code=403)
    ...
    def test_upload_valid(self):
        ...
        new_img = MarkdownImage.objects.filter(
            uploader=self.user, content_type=self.content_type, object_id=None
        ).first()
        ...
        related_img = MarkdownImage.objects.filter(
            uploader=self.user, content_type=self.content_type, object_id=new_img.id
        ).first()
