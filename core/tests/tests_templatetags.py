...
class GenericFieldTemplateTagTest(TestCase):
    def setUp(self):
        form_data = {"test_charfield": "text", "test_select": "A", "test_booleanfield": True}
        self.form = DummyForm(data=form_data)

    # Tests a form with a single charfield
    def test_generic_field_single_charfield(self):
        out = Template("{% load generic_field %}{% generic_field form.test_charfield -1 %}").render(
    ...
    def test_generic_field_single_select(self):
        out = Template("{% load generic_field %}{% generic_field form.test_select -1 %}").render(
            Context({"form": self.form})
        )
    ...
    def test_generic_field_checkboxes(self):
        # Single checkbox
        out = Template("{% load generic_field %}{% generic_field form.test_booleanfield -1 %}").render(
            Context({"form": self.form})
        )
        ...
        # Multiple Checkboxes
        out = Template(
            "{% load generic_field %}{% generic_field form.test_booleanfield form.test_booleanfield -1 -1 %}"
    ...
    def test_generic_field_checkbox_and_other(self):
        # Single checkbox
        out = Template(
            "{% load generic_field %}{% generic_field form.test_booleanfield form.test_charfield -1 -1 %}"
        ).render(Context({"form": self.form}))
    ...
    def test_generic_field_wrong_parameter(self):
        with self.assertRaisesMessage(ImproperlyConfigured, "wrong parameter"):
            ...
            Template("{% load generic_field %}{% generic_field 3 -1 %}").render(Context({"form": self.form}))
    ...
    def test_generic_field_inline_martor(self):
        with self.assertRaisesMessage(
            ...
            # Pass something that isn't actually a field
            Template(
                "{% load generic_field %}{% generic_field form.test_charfield form.test_martorfield -1 -1 %}"
            ).render(Context({"form": self.form}))
...
class BuildAbsoluteURITemplateTagTest(TestCase):
    ...
    def test_build_absolute_uri_not_empty(self):
        request = self.factory.get("/some_location")
        out = Template("{% load build_absolute_uri %}" "{% build_absolute_uri request 'some_cool_url' %}").render(
            Context({"request": request})
        )
