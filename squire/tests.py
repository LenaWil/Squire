...
class GetSecretKeyTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.testcase_nonexisting_key_path = os.path.join(
            settings.BASE_DIR, "test", "input", "testcase_nonexisting_secret_key.txt"
        )
    ...
    def test_get_when_not_exists(self):
        ...
        if os.path.isfile(self.testcase_nonexisting_key_path):
            try:  # If it does, try deleting it
                ...
                self.fail(
                    "The file "
                    + self.testcase_nonexisting_key_path
                    + " should not exist, but it did! Please delete it manually before running the tests again."
                )
