...
class RegisterNewSlotFormTestCase(ActivityFormValidationMixin, TestCase):
    ...
    def test_slot_mode(self, mock_tz):
        ...
        self.assertFormHasError(
            {"sign_up": True, "title": "My slot", "max_participants": -1}, code="user-slot-creation-denied"
        )
    ...
    def test_user_max_slots_occupied(self, mock_tz):
        ...
        Participant.objects.create(user=self.user, activity_slot_id=8)
        self.assertFormHasError(
            {"sign_up": True, "title": "My slot", "max_participants": -1}, code="max-slots-occupied"
        )
    ...
    def test_max_slots_claimed(self, mock_tz):
        ...
        self.activity.save()
        self.assertFormHasError(
            {"sign_up": True, "title": "My slot", "max_participants": -1}, code="max-slots-claimed"
        )
    ...
    def test_save_form_creation(self, mock_tz):
        ...
        slot_query = ActivitySlot.objects.filter(parent_activitymoment=self.activity_moment, title="My slot")
    ...
    def test_save_form_creation_without_user(self, mock_tz):
        ...
        slot_query = ActivitySlot.objects.filter(parent_activitymoment=self.activity_moment, title="Some slot")
        self.assertFalse(slot_query.exists())
        form.save()
        self.assertTrue(slot_query.exists())
