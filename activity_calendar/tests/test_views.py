...
class TestActivityViewMixin:
    ...
    def setUp(self):
        ...
        self.activity_moment, _ = ActivityMoment.objects.get_or_create(
            parent_activity_id=self.activity.id, recurrence_id=self.recurrence_id
        )
    ...
    def build_post_response(self, data, iso_dt=None, url_name=None, activity_id=None, follow=False):
        return self._build_response(
            "post", iso_dt=iso_dt, url_name=url_name, activity_id=activity_id, follow=follow, data=data
        )
...
class ActivityMixinTest(TestMixinMixin, TestCase):
    ...
    def test_can_edit_activity_through_permission(self):
        ...
        User.objects.get(id=2).user_permissions.add(
...
class ActivitySimpleViewTest(TestActivityViewMixin, TestCase):
    ...
    def test_successful_post(self, mock_tz):
        ...
        self.assertTrue(
            ActivityMoment.objects.get(
                parent_activity_id=self.default_activity_id,
                recurrence_id=self.recurrence_id,
            )
            .get_user_subscriptions(self.user)
            .exists()
        )
...
class ActivitySlotViewTest(TestActivityViewMixin, TestCase):
    ...
    def test_successful_post(self, mock_tz):
        ...
        self.assertTrue(ActivitySlot.objects.filter().exists())
        msg = _("You have successfully been removed from '{activity_name}'").format(
            activity_name="Liberty the boardgame"
        )
        self.assertHasMessage(response, level=messages.SUCCESS, text=msg)
    ...
    def test_invalid_post(self, mock_tz):
        # Can not unsubscribe from an activity you are not registered to.
        response = self.build_post_response({"sign_up": True, "slot_id": 8}, follow=True)

        self.assertRedirects(response, self.base_url)  # Should redirect to prevent resending the post on page refresh
        self.assertTrue(ActivitySlot.objects.filter().exists())
        self.assertHasMessage(
            response, level=messages.ERROR, text=ActivityMomentWithSlotsView.error_messages["slot-full"]
        )
...
class CreateSlotViewTest(TestActivityViewMixin, TestCase):
    ...
    def test_unsuccesfull_post(self, mock_tz):
        ...
        response = self.build_post_response({}, follow=False)
        self.assertEqual(response.status_code, 200)
        self.assertIn("form", response.context)
...
class CancelActivityMomentViewTest(TestActivityViewMixin, TestCase):
    ...
    def test_successful_post(self):
        ...
        # Assert that the instance is saved
        activitymoment = ActivityMoment.objects.get(parent_activity_id=2, recurrence_id=local_datetime)
